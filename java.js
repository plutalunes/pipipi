...
let playerXIcon = 'minini.png'; // Путь к изображению для X
let playerOIcon = 'miniku.png'; // Путь к изображению для O
let playerSign = "X";
let runBot = true;
// Initialize the game
window.onload = () => {
...
function clickedBox(element) {
    if (players.classList.contains("player")) {
        ...
        element.innerHTML = `<img src="${playerOIcon}" alt="O" class="icon">`; // Add 'O' icon
        players.classList.remove("active"); // Switch active player
    } else {
        element.innerHTML = `<img src="${playerXIcon}" alt="X" class="icon">`; // Add 'X' icon
    ...
    if (runBot) {
        ...
        if (randomBox) {
            if (players.classList.contains("player")) {
                ...
                randomBox.innerHTML = `<img src="${playerXIcon}" alt="X" class="icon">`;
                players.classList.add("active");
            } else {
                playerSign = "O"; // Bot plays 'O' if player chose 'X'
                randomBox.innerHTML = `<img src="${playerOIcon}" alt="O" class="icon">`; 
